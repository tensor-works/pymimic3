...
class RunningAvgMetric():

    def __init__(self, metric: metric.Metric[torch.Tensor] = None):
        self._metric = metric
    ...
    def __repr__(self):
        return f"RunningAvg{self._metric.__name__}(count={self._count}, average={self.compute():.4f})"


class RunningAvgLoss():
    ...
    def __repr__(self):
        return f"RunningAvgLoss(count={self._count}, average={self.compute():.4f})"
