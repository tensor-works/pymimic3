...
def compact_processing(
...
) -> Dict[str, Dict[str, Dict[str, pd.DataFrame]]]:  # return might be incorect
    """_summary_
    ...
    info_io("Compact Preprocessing", level=0)
    ...
    if num_subjects is not None:
        ...
        while not len(X_subjects) == num_subjects:
            ...
            X, y = preprocessor.transform(dataset=dataset)
            X_subjects.update(X)
            y_subjects.update(y)
...
def iterative_processing(reader: ExtractedSetReader,
    ...
    info_io("Iterative Preprocessing", level=0)
...
def get_subject_ids(num_subjects: int,
    ...
    if num_subjects is not None:
        ...
        unknown_subjects = (set(remaining_subject_ids) - set(subject_ids)
                           ) & set(set(remaining_subject_ids) - set(processed_subjects))
        if unknown_subjects:
