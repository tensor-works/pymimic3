...
@pytest.mark.parametrize("extraction_style", ["iterative", "compact"])
@pytest.mark.parametrize("task_name", TASK_NAMES)
def test_subject_ids_engineer_only(task_name: str, extraction_style: str):
...
@pytest.mark.parametrize("extraction_style", ["iterative", "compact"])
@pytest.mark.parametrize("task_name", TASK_NAMES)
def test_subject_ids_engineer(task_name: str, extraction_style: str):
    ...
    for extraction_style in ["compact", "iterative"]:
        for task in TASK_NAMES:
            ...
            for task in TASK_NAMES:
                ...
                if TEMP_DIR.is_dir():
                    shutil.rmtree(str(TEMP_DIR))
                test_subject_ids_engineer_only(task, extraction_style)
                if TEMP_DIR.is_dir():
                    shutil.rmtree(str(TEMP_DIR))
                test_subject_ids_preprocessing(task, subjects, extraction_style)
                if TEMP_DIR.is_dir():
                    shutil.rmtree(str(TEMP_DIR))
                test_subject_ids_engineer(task, extraction_style)
